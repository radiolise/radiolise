...
window.onoffline = function() {
  if (!player.paused) {
    UI.message(tr("Oh no! The stream was interrupted due to a sudden disconnect."));
  }
  Player.stream.stop();
...
hls.on(Hls.Events.ERROR, function(_, data) {
  Player.stream.stop();
  Player.update.setName(nostream);
  UI.message(tr("Sorry, an error has occurred. ") + ((Hls.isSupported()) ? ((data.details != "levelLoadTimeOut") ? data.details : tr("Timeout: Your network connection seems to be too slow at the moment.")) : tr("The stream may need a protocol like HLS, which doesn’t seem to be supported by your browser.")));
});
...
function removeList(name) {
  ...
  Data.offerUndeletion(tr("List named ‘") + name + tr("’ has been removed."), listbackup, "list", applyLists);
}
