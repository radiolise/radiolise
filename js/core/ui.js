...
const UI = {
  ...
  hint: {
    ...
    show: function(text, square, confirm) {
      ...
      var timems = 5000;
      if (confirm) {
        $("#hint > div").append("<div style='text-align: right'><a onclick='UI.hint.hide()' class='button'>" + tr("OK") + "</a></div>")
      }
      else if (text != "load") {
    ...
    open: function(id) {
      ...
      //     });
        ...
        if (id == "addstation") {
          ...
          UI.stationManager.refreshTagItems(currentlist[gearclicked].tags || "");
        }
        else if (id == "history") {
        ...
        for (var i = titles[type].length - 1; i >= 0; i--) {
          ...
          if (type == "favorites") {
            ...
            if (month != currentmonth) {
              content += "<div class='monthseparator' style='margin: 10px 0; padding: 5px 10px; font-size: 20px; font-weight: 500; opacity: .7'>" + currentmonth + "</div>";
              month = moment(item.time * 60).format("MMMM YYYY");
            }
          }
          content += "<div class='listitem' style='padding: 10px'><div style='margin-bottom: 5px; font-weight: normal; font-size: 17px; font-weight: 500'>" + item.info + "</div><div style='opacity: .7'><span style='float: left'>" + item.station + "</span>&nbsp;<span data-time='" + item.time + "' style='float: right'>" + moment(item.time * 60).fromNow() + "</span></div><div class='options' style='display: none'>" + (type == "favorites" ? "<a class='button' href='https://musicbrainz.org/search?query=" + encodeURIComponent(item.info) + "&type=recording' target='_blank' style='margin-bottom: 0'><i class='fa fa-fw fa-search'></i><span tr>MusicBrainz</span></a><a class='button' style='margin-bottom: 0' onclick='Data.offerUndeletion(\"" + tr("‘") + item.info + tr("’ has been removed from bookmarks.") + "\", titles.favorites.slice(), \"bookmarks\", function() {Data.refresh(); UI.titleManager.insertList(\"favorites\")}); titles.favorites.splice(" + i + ", 1); Data.refresh(); UI.titleManager.insertList(\"favorites\")'><i class='fa fa-fw fa-times'></i><span>" + tr("Remove") + "</span></a>" : "<a class='button' style='margin-bottom: 0' onclick='Data.createBookmark(" + JSON.stringify(item) + ")'><i class='fa fa-music'></i><i class='fa fa-fw fa-xs fa-plus' style='vertical-align: super; margin: 0'></i><span>" + tr("Bookmark") + "</span></a><a class='button' href='https://musicbrainz.org/search?query=" + encodeURIComponent(item.info) + "&type=recording' target='_blank' style='margin-bottom: 0'><i class='fa fa-fw fa-search'></i><span tr>MusicBrainz</span></a>") + "</div></div>";
        }
      }
      else {
        content += tr("<p style='padding: 0; text-align: center'>" + tr("This list is currently empty.") + "</p>");
      }
      $("#" + (type == "favorites" ? "favorites" : "recent")).html(content).data("content", JSON.stringify(titles[type]));
      if (type == "favorites") {
        var current = titles.favorites[titles.favorites.length - 1];
        if (!current || current.station != currentstation || current.info != info) {
